...
namespace AntiDupl.NET.Controls
{
	public class ColoredTextBox : TextBox
	{
		...
		public Color BorderDefaultColor { get; set; } = SystemColors.ControlDark;
		...
		protected override void WndProc(ref Message m)
		{
			...
			if (m.Msg == WM_NCPAINT && BorderColor != Color.Transparent && BorderStyle == System.Windows.Forms.BorderStyle.Fixed3D) {
				var hdc = GetWindowDC(this.Handle);
				using (var pen = new Pen(BorderColor)) {
					Graphics.FromHdcInternal(hdc).DrawRectangle(pen, new Rectangle(0, 0, Width - 1, Height - 1));
				}
